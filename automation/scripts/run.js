...
async function expandAll(page) {
  ...
  for (let i = 0; i < (config.expand.maxClicks || 30); i++) {
    ...
    for (const t of texts) {
      const btn = page.getByText(t, { exact: false }).filter({ has: page.locator('button, a, div, span') });
      const count = await btn.count();
      if (count > 0) {
        for (let j = 0; j < count; j++) {
          try {
            await btn.nth(j).click({ timeout: 1000 });
            await sleep(500);
            didClick = true;
            clicked++;
          } catch (_) {}
...
async function extractFeishuLink(page) {
...
async function extractRegionByAuthorClick(context, articlePage, authorName) {
  ...
  const nameNode = articlePage.getByText(authorName || '', { exact: false }).first();
  if ((await nameNode.count()) === 0) return unknown;

  const clickTarget = nameNode.locator('xpath=ancestor::*[contains(@class,"name-identity") or contains(@class,"post-item-top-right")][1]');
  const target = (await clickTarget.count()) > 0 ? clickTarget.first() : nameNode;
  ...
  if (popup) {
    ...
    const region = await extractRegionFromAuthorPage(popup);
    await popup.close().catch(() => {});
    return region || unknown;
  }

  await articlePage.waitForURL((u) => u.toString() !== previousUrl, { timeout: 5000 }).catch(() => {});
...
(async () => {
  ...
  for (let i = 0; i < sectionCount; i++) {
    ...
    for (let c = 0; c < cardCount; c++) {
      ...
      let feishuLink = await extractFeishuLink(articlePage);

      let region = formSelectors.unknownRegionText || '暂时不知';
      if (selectors.authorLink || author) {
